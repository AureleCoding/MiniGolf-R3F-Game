...
function Grass({options = {bW: 0.01, bH: 0.05, joints: 5}, width = 10, instances = 200000, ...props}) {
    const {bW, bH, joints} = options
    const materialRef = useRef()
    const [texture, alphaMap] = useLoader(THREE.TextureLoader, ["/textures/blade_diffuse.jpg", "textures/blade_alpha.jpg"])
    ...
    return (
        <group {...props}>
            <mesh>
                <instancedBufferGeometry index={baseGeom.index} attributes-position={baseGeom.attributes.position}
                    ...
                    <instancedBufferAttribute attach={"attributes-orientation"}
                                              args={[new Float32Array(attributeData.orientations), 4]}/>
                    <instancedBufferAttribute attach={"attributes-stretch"}
                                              args={[new Float32Array(attributeData.stretches), 1]}/>
                    <instancedBufferAttribute attach={"attributes-halfRootAngleCos"}
                                              args={[new Float32Array(attributeData.halfRootAngleCos), 1]}/>
                    <instancedBufferAttribute attach={"attributes-halfRootAngleSin"}
...
function getAttributeData(instances, width) {
    ...
    for (let i = 0; i < instances; i++) {
        ...
        if (offsetX < 0.5 && offsetX > -0.5 && offsetZ < 0.5 && offsetZ > -3.5) {
            i--
            continue
        }
